...
| 对比项 | bs.schedule | D-Better-Schedule | doom.schedule |
|------|------------|------------------|--------------|
| 函数数 | ~20 | ~60 | **35** |
...
`mcdoc/doom.schedule.mcdoc` 提供 `doom.schedule:data`、`doom.schedule:const`、`doom.schedule:ctx` 三个 storage 的结构补全。示例见 `function/mcdoc.mcfunction`，需 Spyglass 或 `Misodee.vscode-mcdoc` 插件。
